...
export default function App() {
  ...
  const loadAppData = () => {
    ...
    getSystemStatus().then((payload) => { setSystemStatus(payload); setSystemStatusError('') }).catch((err: unknown) => {
      if (err instanceof ApiError && err.status === 401) {
        setAuthMode('login'); setAuthError('Your session has expired. Please log in again.'); return
      }
      if (err instanceof ApiError && err.status === 403) {
        setSystemStatusError('You do not have permission to perform this action.'); return
      }
  ...
  const bootstrapAuth = async () => {
    ...
    try {
      ...
      if (setup.setup_required) { setAuthMode('setup'); return }
      try {
        await getMe()
        setAuthMode('app')
        loadAppData()
      } catch (err: unknown) {
        if (err instanceof ApiError && err.status === 401) { setAuthMode('login'); return }
        setAuthMode('error'); setAuthError('Authentication state could not be loaded.')
      }
    } catch {
      setAuthMode('error'); setAuthError('Setup state could not be loaded.')
    }
  }
  ...
  const onSetupSubmit = async (payload: { username: string; email?: string; password: string; password_confirm: string }) => {
    setAuthError('')
    try {
      const res = await setupAdmin(payload)
      if (res.user) { await bootstrapAuth(); return }
      setAuthMode('login')
    } catch (err: unknown) { setAuthError(err instanceof Error ? err.message : 'Setup failed') }
  }
